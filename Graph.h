...
class graph {
  ...
  public:
    ...
    struct vertex_label {
      ...
      char state;
      int npaths;
      int inc;              // added inc variable to store incoming edges
      int out;              // added out variable to store outcoming edges

      
      vertex_label( double _dist=0.0, int _pred=-1, char _state='?',
          int _npaths=0) 
        : dist { _dist }, pred { _pred }, state { _state}, npaths { 0 }, inc { 0 }, out{ 0 }
      { }
  ...
  public:
    ...
    bool bfs(int src, std::vector<vertex_label> &report) {
      ...
      while(!q.empty()) {
        ...
        for(edge &e : vertices[u].outgoing) {
          v = e.vertex_id;
          if(report[v].state == UNDISCOVERED) {  // if the node is undiscovered
            report[v].dist = report[u].dist + 1; // we discover it
            report[v].pred = u;
            report[v].state = DISCOVERED;
            // enqueue newly discovered vertex
            q.push(v);                          // push to stack
          }
          // record the number of shortest paths
          if(report[v].state == DISCOVERED) {
              if (report[v].dist == report[u].dist + 1)  // if the distance of current is equal to next
                  report[v].npaths += report[u].npaths; // sum npaths
              else if (report[v].dist > report[u].dist + 1) {
                  report[v].npaths = report[u].npaths;      // if above not true, set equal
                  report[v].dist = report[u].dist + 1;      // add distance+1
              }
          }
        }
      }
  ...
  public:
    ...
    bool extract_path(const vector<vertex_label> & rpt, 
      ...
      if(rpt.size() != num_nodes())
        return false;
      // your code here!
      // if source is destination, source is destination of itself
      ...
      if(rpt[dest].pred == -1)
        return false;
      //
      //cout << "debug: dest.pred" <<rpt[dest].pred << endl;
      _ept_help(rpt, dest, path);       // helper function so it's possible to call recursively
      return true;  // placeholder
    }

private:
    // recursive function so it's possible to add to vector in the inverse of inverse order
    void _ept_help(const vector<vertex_label> &rpt, int dest, vector<int> &path){
        if(rpt[dest].pred == dest){     // if source = dest
            path.push_back(dest);       // push back dest to the vector
            return;                     // break the function
        }
        //else{
            _ept_help(rpt, rpt[dest].pred, path);   // call recursively so we go towards source
            path.push_back(dest);                   // add to vector
        //}
...
public:
    ...
    bool dag_critical_paths(vector<vertex_label> & rpt) {
      ...
      if(has_cycle())
        return false;
      // your code here...
      vector<int> crit_paths;           // create a vector
      init_report(rpt);                 // init report the vector
      topo_sort(crit_paths);            // topo-sort and populate the vector

      //for(auto &i : crit_paths){                            // for each element in the vector
      for(int i = 0; i < crit_paths.size(); i++){
          if(vertices[crit_paths[i]].incoming.empty()){     // if vertex has no incoming edges
              rpt[crit_paths[i]].dist = 0;                     // set to distance = 0
              rpt[crit_paths[i]].pred = crit_paths[i];      // set pred to itself
              //cout<<"DEBUG crit_paths: " << i << " " << rpt[crit_paths[i]].pred << endl;
          }

          for(edge &e : vertices[crit_paths[i]].outgoing){      // for each outgoing edge in vector
              int v = e.vertex_id;
              double w = e.weight;
              if(rpt[v].dist < rpt[crit_paths[i]].dist + w){    // if distance of current vertex_id
                rpt[v].dist = rpt[crit_paths[i]].dist + w;      // is less than dist + weight of edge
                rpt[v].pred = crit_paths[i];                    // set precedent to current edge
              }
          }
      }
      return true;
...
public:
    bool dag_num_paths(vector<vertex_label> & rpt) {
      if(has_cycle())
        return false;
      // your code here...
      init_report(rpt);             // init report the rpt
      // get incoming and outgoing edges
      int i = 0;
      int j = num_nodes();
      for(i = 0; i < j; i++) {            // for all nodes that there is
          if (vertices[i].incoming.empty())         // if current node has no incoming edges
              out_help(i, rpt);                     // get edges with a helper function
      }
        for(i = 0; i < j; i++) {
            if (vertices[i].outgoing.empty())         // if current node has no outcoming edges
                inc_help(i, rpt);                     // get edges with a helper function
        }
        for(i = 0; i < j; i++) {
            rpt[i].npaths = rpt[i].inc * rpt[i].out;  // set npaths as multiple of incoming x outcoming
      }
        return true;
    }

private:
    int out_help(int value, vector<vertex_label> &rpt) {     // outgoing-edges helper function
        if (vertices[value].outgoing.empty())                // if outgoing empty
            rpt[value].out = 1;                             // set = 1

        else {
            for (int i = 0; i < vertices[value].outgoing.size(); i++) {   // for all outgoing edges
                int a = vertices[value].outgoing[i].vertex_id;      // set a = vertex_id
                if (rpt[a].out > 0) {
                    rpt[value].out += rpt[a].out;                   // increase outgoing
                    rpt[value].npaths = rpt[value].out;             // npaths = outgoing edges
                } else
                    rpt[value].out += out_help(a, rpt);             // else: recursively add
            }
        }
        return rpt[value].out;
    }
    int inc_help(int value, vector<vertex_label> &rpt){
        if (vertices[value].incoming.empty())                // if incoming empty
            rpt[value].inc = 1;                             // set = 1

        else {
            for (int i = 0; i < vertices[value].incoming.size(); i++) {  // for all incoming edges
                int a = vertices[value].incoming[i].vertex_id;      // set a = vertex_id
                if (rpt[a].inc > 0) {
                    rpt[value].inc += rpt[a].inc;                   // increase incoming
                    rpt[value].npaths = rpt[value].inc;             // npaths = incoming edges
                } else
                    rpt[value].inc += inc_help(a, rpt);             // else: recursively add
            }
        }
        return rpt[value].inc;
    }
...
public:
     ...
     */
private:
// I'll use a structure for the vector in the function below
struct ordered_map{
    //bool is_present;
    int is_present;
};
public:
    ...
    bool valid_topo_order(const vector<int> & order) {
      if(has_cycle())               // base case
        return false;
      vector<ordered_map> ord_map;  // create a vector from the struct above
      ord_map.resize(order.size()); // resize the vector to match order
      int temp, i, j;               // variables for loop and tracking

      for(i = 0; i < order.size(); i++) {     // mark
          if (vertices[order[i]].incoming.empty())
              ord_map[order[i]].is_present = -1;     // if the vector of order i is empty -> mark

          for (j = 0; j < vertices[order[i]].incoming.size(); j++) { // loop through all edges
              temp = vertices[order[i]].incoming[j].vertex_id;       // assign a temporary var the id
              if (ord_map[temp].is_present != -1)                          // if the incoming is not present
                  return false;
          }
        ord_map[order[i]].is_present = -1;                        // mark the current vector
      }
      return true;
    }
    ...
    bool enum_paths(int target, vector<string> &paths) {
      ...
      for(int i = 0; i < vertices.size(); i++){
          if(vertices[i].incoming.empty())
            enum_help(target, path, i, paths);
      }
      return true;
    }
private:
    void enum_help(int target, string path, int value, vector<string> &paths){
        bool is_path = false;           // bool value to use recursively
        int track;              // value to return recursively
        if(value == target)
            paths.push_back(path + " " + vertices[value].name);
        else{
            for(edge &e : vertices[value].outgoing){
                track = e.vertex_id;
                if(!is_path)
                    path += " " + vertices[value].name;
            }
            enum_help(is_path, path, track, paths);
            is_path = true;
        }
