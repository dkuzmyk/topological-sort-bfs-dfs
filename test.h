    ...
    bool bfs(int src, std::vector<vertex_label> &report) {
        ...
        while(!q.empty()) {
            ...
            for(edge &e : vertices[u].outgoing) {
                ...
                // use this for npaths
                // it will revisit to keep track number of paths and all we do is compare the distance if we wantt to incr the number of shortest path
...
public:
    ...
    bool extract_path(const vector<vertex_label> & rpt,
        ...
        // loop through dest
        // this adds to the vector backwards!!!!
        // while (rpt[dest].pred != dest) {
        //     path.push_back(dest);
        //     dest = rpt[dest].pred;
        //
        //     if (rpt[dest].pred == dest) {
        ...
        extractPathHelper(rpt, dest, path);  // recursive helper function to add to vector in the proper order
...
public:
    bool dag_critical_paths(vector<vertex_label> & rpt) {
        ...
        // version 1 to solve this funcstion below
        ...
        //         criticalPATH.push_back(i);              // store all the vertex with no incoming
        //     }
        // }
        //
        //
        // //topo_sort(criticalPATH);  // topo sort that shit
        //
        // //cout << vertices[1].outgoing[0].weight << "BOOO" << endl;
        //
        // // loop through all vertexes with no incoming
        // for (int i=0; i < criticalPATH.size(); i++) {
        //     rpt[criticalPATH[i]].dist = 0;
        //     q.push(criticalPATH[i]);
        //     rpt[criticalPATH[i]].pred = criticalPATH[i];
        //     rpt[criticalPATH[i]].state = FINISHED;
        //
        //     while(!q.empty()) {
        //       // dequeue front node from queue
        //       u = q.front();
        ...
        //
        //           if (rpt[v].dist < rpt[u].dist + weight ) {
        //                rpt[v].dist = rpt[u].dist + weight;
        //                rpt[v].pred = u;
        //           }
        //           // enqueue newly discovered vertex
        //           rpt[v].state = FINISHED;
        //           q.push(v);
        //       }
        //     }
        //
...
public:
    bool dag_num_paths(vector<vertex_label> & rpt) {
        ...
        // get all numbr of outgoing edges
        ...
        // get all incoming number of edges
        for (int i= 0; i< num_nodes(); i++){
...
private:
    ...
    int incoming(int source, vector<vertex_label> &rpt){
        ...
        // incoming of a node with no inocming is 1
        if(vertices[source].incoming.size() == 0){
            ...
            for (int i = 0; i< vertices[source].incoming.size(); i++){
                ...
                if(rpt[v].numIncoming > 0){
                    rpt[source].numIncoming += rpt[v].numIncoming;
                    rpt[source].npaths = rpt[source].numIncoming;

                }
                else{
                    // make the recursive call for the next one
                    rpt[source].numIncoming += incoming(v, rpt);
                }
            }
        }
    ...
    int outgoing(int source, vector<vertex_label> &rpt){
        ...
        if(vertices[source].outgoing.size() == 0){
            ...
            for (int i = 0; i< vertices[source].outgoing.size(); i++){
                ...
                if(rpt[v].numOutgoing > 0){
                    rpt[source].numOutgoing += rpt[v].numOutgoing;
                    rpt[source].npaths = rpt[source].numOutgoing;
                }
                else{
                    rpt[source].numOutgoing += outgoing(v, rpt);
                }
            }
        }
...
public:
    bool valid_topo_order(const vector<int> & order) {
        ...
        //  cout << endl;
        //
        //  for (int j = 0; j < vertices[9].incoming.size(); j++) {
        //      cout << vertices[9].incoming[j].vertex_id;
        //  }
...
public:
    bool enum_paths(int target, vector<string> &paths) {
           ...
           path.clear();  // clear out existing memory string


           // loop through to find all the vertex with incoming size 0 so we can start from there
           for (int i =0; i < vertices.size(); i++ ) {

               if (vertices[i].incoming.size() == 0) {
                   enumHelp(target, path, i, paths  );  // recursive helper function
               }
...
private:
       void enumHelp(int target, string path, int KevinDurant, vector<string> &paths) {
         int v;
         bool LebronJamesThrowsItDown = false ;

         // base case
         if (KevinDurant == target) {
             //cout << "Hello" << endl;
             paths.push_back(path + " " +  vertices[KevinDurant].name);
         }

         else {

             for(edge &e : vertices[KevinDurant].outgoing) {

                 v = e.vertex_id;

                 if (LebronJamesThrowsItDown == false) {
                     cout << path << endl; // its this part keeps print out v1 and v2 in the same location
                     path = path + " " + vertices[KevinDurant].name;
                 }

                 enumHelp(LebronJamesThrowsItDown, path, v, paths);
                 LebronJamesThrowsItDown = true;
             }
